...
\label{chap:modmet}
To produce results for the thesis, a formulation of the Weather Research and Forecasting (WRF) Model called the Advanced Research WRF (ARW) has been used. The model is described in the first part of this chapter. Then follows a brief description of the schemes treating cloud microphysical processes and radiation, that were applied in this study. Model setup and runs are summarized before two short sections on the input data and processing of the model output at the very end of this chapter.
...
\section{Input data}
\label{sec:inputdata}
The model runs were initialized with data downloaded from the European Centre for Medium-Range Weather Forecasts (ECMWF)~\citet{ecmwf}%@cite web page.%was downloaded from their site and used as input for initial and boundary?? conditions.
